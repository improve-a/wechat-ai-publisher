...
import { planLegacyMappingBaseline } from "../editorial-acceptance";
...
export function EditorialAcceptanceV2Page() {
  ...
  const branch = params.get("branch") === "baseline" ? "baseline" : "art-direction";
  ...
  return (
    <main className="m5-preview-shell" data-testid="editorial-v2-page">
      <header className="m5-preview-header">
        ...
        <h1>{fixture.article.title} · {branch === "baseline" ? "规则映射基线" : "Art Direction 编排"}</h1>
      ...
      <section className="m5-validator-status">
        ...
        >
          cards={result.stats.cardSurfaceCount}; full-width={result.stats.fullWidthImageCount}; groups={result.stats.photoGroupCount}; asym={result.stats.asymmetricCompositionCount}; treatments={result.stats.uniqueImageTreatmentCount}
        </span>

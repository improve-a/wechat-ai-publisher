...
function Photo({ spec, height, width = "100%" }: { spec: ArtworkSpec; height: number; width?: string }) {
  ...
  if (!image) return <div data-artwork-photo="none" style={{ width, height, background: "#e9e5dc" }} />;
  return (
    <div data-artwork-photo={image.sourceAssetId} data-artwork-image-fit="contain" style={{ width, height, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: "#e9e5dc" }}>
      <img src={image.src} alt={image.alt} style={{ display: "block", maxWidth: "100%", maxHeight: "100%", width: "auto", height: "auto", objectFit: "contain" }} />
    </div>
  );
}
  ...
  return (
    <section
      ...
      style={{
        ...
        background: stylePack.palette.paper, color: stylePack.palette.ink,
        fontFamily: stylePack.typographyHierarchy.fontFamily,
      }}
    >
      {children}
    </section>
  ...
  return (
    <Frame spec={spec}>
      ...
      <div style={{
        ...
        background: isSplit ? pack.palette.ink : pack.palette.paper,
        color: isSplit ? pack.palette.paper : pack.palette.ink,
      }}>
        <p style={{ margin: "0 0 6px", fontSize: 11, fontWeight: 800, letterSpacing: "0.16em", color: isSplit ? pack.palette.accentSoft : pack.palette.accent }}>BIT · EDITORIAL</p>
        <h1 style={{ margin: 0, fontSize: isSplit ? 22 : 20, lineHeight: 1.25, fontWeight: 800, letterSpacing: "-0.03em" }}>{title}</h1>
        {subtitle ? <p style={{ margin: "8px 0 0", fontSize: 11, lineHeight: 1.45, color: isSplit ? "#d6dce5" : pack.palette.muted }}>{subtitle}</p> : null}
      </div>
    </Frame>
  );
}
  ...
  return (
    <Frame spec={spec}>
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "stretch" }}>
        <div style={{ width: "34%", padding: "20px 16px", boxSizing: "border-box", background: pack.palette.ink, color: pack.palette.paper }}>
          <p style={{ margin: 0, fontSize: 11, letterSpacing: "0.14em", color: pack.palette.accentSoft }}>KEY TRANSITION</p>
          <Rule color={pack.palette.accent} width={34} />
        </div>
        <div style={{ width: "66%", boxSizing: "border-box", padding: "18px 22px", background: pack.palette.paper }}>
          <h2 style={{ margin: 0, fontSize: 21, lineHeight: 1.32, fontWeight: 800 }}>{title}</h2>
          {subtitle ? <p style={{ margin: "8px 0 0", fontSize: 11, lineHeight: 1.45, color: pack.palette.muted }}>{subtitle}</p> : null}
        </div>
      </div>
    </Frame>
  );
  ...
  return (
    <Frame spec={spec}>
      <div style={{ display: "flex", width: "100%", height: "100%", alignItems: "stretch" }}>
        ...
        <div style={{ boxSizing: "border-box", width: "52%", padding: "34px 24px 24px", background: pack.palette.paper }}>
          <p style={{ margin: "0 0 16px", fontSize: 11, letterSpacing: "0.14em", color: pack.palette.accent }}>PROFILE · FIELD NOTE</p>
          <h2 style={{ margin: "0 0 14px", fontSize: 23, lineHeight: 1.3, fontWeight: 800 }}>{title}</h2>
          {identity ? <p style={{ margin: "0 0 12px", paddingTop: 10, borderTop: `1px solid ${pack.palette.line}`, fontSize: 11, lineHeight: 1.55, color: pack.palette.ink }}>{identity}</p> : null}
          {subtitle ? <p style={{ margin: 0, fontSize: 11, lineHeight: 1.6, color: pack.palette.muted }}>{subtitle}</p> : null}
        </div>
      </div>
    </Frame>
  );
...
function AchievementArtwork({ spec }: { spec: ArtworkSpec }) {
  ...
  return (
    <Frame spec={spec}>
      <div style={{ boxSizing: "border-box", width: "100%", height: "100%", padding: "24px 26px", background: pack.palette.paper, display: "flex", gap: 22 }}>
        <div style={{ width: "56%" }}><Photo spec={spec} height={152} /></div>
        <div style={{ width: "44%", paddingTop: 6 }}>
          <p style={{ margin: "0 0 12px", fontSize: 11, letterSpacing: "0.14em", color: pack.palette.accent }}>EVIDENCE · ACHIEVEMENT</p>
          <h2 style={{ margin: "0 0 12px", fontSize: 22, lineHeight: 1.3, fontWeight: 800 }}>{title}</h2>
          {subtitle ? <p style={{ margin: 0, fontSize: 11, lineHeight: 1.5, color: pack.palette.muted }}>{subtitle}</p> : null}
        </div>
      </div>
    </Frame>
  );
...
function QuoteArtwork({ spec }: { spec: ArtworkSpec }) {
  ...
  return (
    <Frame spec={spec}>
      <div style={{ boxSizing: "border-box", width: "100%", height: "100%", padding: "26px 36px 22px", background: pack.palette.ink, color: pack.palette.paper }}>
        <span aria-hidden="true" style={{ display: "block", height: 34, fontFamily: "Georgia,serif", fontSize: 52, lineHeight: 1, color: pack.palette.accentSoft }}>“</span>
        <blockquote style={{ margin: 0, maxWidth: 310, fontSize: 18, lineHeight: 1.6, fontWeight: 650 }}>{quote}</blockquote>
        <div style={{ marginTop: 14 }}><Rule color={pack.palette.accent} /></div>
      </div>
    </Frame>
  );
}
...
function ClosingArtwork({ spec }: { spec: ArtworkSpec }) {
  ...
  return (
    <Frame spec={spec}>
      ...
      <div style={{ boxSizing: "border-box", height: 78, padding: "15px 24px", background: pack.palette.ink, color: pack.palette.paper }}>
        <p style={{ margin: "0 0 5px", fontSize: 11, letterSpacing: "0.15em", color: pack.palette.accentSoft }}>CLOSING SCENE</p>
        <p style={{ margin: 0, fontSize: title ? 13 : 15, lineHeight: 1.45, fontWeight: 700 }}>{title ?? closing}</p>
        {title && closing ? <p style={{ margin: "4px 0 0", fontSize: 11, lineHeight: 1.4, color: "#d6dce5" }}>{closing}</p> : null}
      </div>
    </Frame>
  );
}
